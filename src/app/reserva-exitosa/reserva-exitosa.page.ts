...
@Component({
  ...
  templateUrl: './reserva-exitosa.page.html',
  styleUrls: ['./reserva-exitosa.page.scss'],
})
export class ReservaExitosaPage implements OnInit {
  ...
  get_relative_date () {
    if (this.date.isSame (moment (), 'day')) {
      return 'Hoy';
    } else if (this.date.isSame (moment().add (1, 'days'), 'day')) {
      return 'Mañana';
    }

    let day: string = this.date.format ('dddd');
    return day.charAt(0).toUpperCase () + day.slice (1);
  }

  get_date () {
    let month: string = this.date.format ('MMM');
    return this.date.format ('DD[ ]') + month.charAt(0).toUpperCase () + month.slice (1);
  }
